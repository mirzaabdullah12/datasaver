...
export default function App() {
  ...
  // customer search — no useEffect, computed inline to avoid infinite loop
            ...
            {custResults.length > 0 && (
              <div style={{ marginTop:6, background:'var(--card-bg)', border:'1px solid var(--border)', borderRadius:8, boxShadow:'0 4px 12px rgba(0,0,0,0.1)', maxHeight:180, overflowY:'auto' }}>
                {custResults.map((name, i) => (
                  <div key={i}
                    onClick={() => { setViewCustomer(name); setCustSearch(''); setSideOpen(false) }}
                ...
                <div className="chart-card">
                  ...
                  <ResponsiveContainer width="100%" height={200}>
                    <AreaChart data={monthlyData}>
                      ...
                      <Area type="monotone" dataKey="leasing" name="Leasing" stroke="#16a34a" strokeWidth={2.5} fill="url(#gL)" />
                      <Area type="monotone" dataKey="device" name="Device" stroke="#22c55e" strokeWidth={2.5} fill="url(#gD)" />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
                <div className="chart-card">
              ...
              <div className="chart-card bar-chart-row">
                ...
                <ResponsiveContainer width="100%" height={140}>
                  <BarChart data={monthlyData} barSize={28}>
                    ...
                    <Tooltip content={<ChartTip />} />
                    <Bar dataKey="count" name="Approvals" fill="#16a34a" radius={[6,6,0,0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </>
          ...
          <div className="table-card">
            <div className="table-hdr">
              <div>
                ...
                <p>Showing {owner}'s records — click any customer name to view details</p>
              </div>
        ...
        <div className="modal-overlay" onClick={() => setViewCustomer(null)}>
          <div className="modal cust-modal" onClick={e => e.stopPropagation()}>
            <div className="modal-hdr">
              <div>
                <h2>👤 {viewCustomer}</h2>
                <p>{custRecs.length} record{custRecs.length !== 1 ? 's' : ''} found</p>
              </div>
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-hdr">
              <div>
                ...
                <p>Fill in the details — everything can be edited later</p>
              </div>
